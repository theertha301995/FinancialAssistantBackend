// File: src/controllers/aiChatController.ts
// 100% FREE VERSION with Multilingual Support
...
import { 
  ...
  detectLanguage,
  getLanguageName 
} from "../services/transalationService";
 ...
 */
const generateSimpleResponse = (amount: number, category: string, familyTotal: number): string => {
  const responses = [
 ...
 */
export const logExpenseByChat = async (
  req: AuthRequest,
  res: Response,
  next?: NextFunction
  ...
  try {
    ...
    logger.info("Processing chat expense with multilingual NLP", {
      userId: req.user.id,
    ...
    console.log(`📝 Original (${getLanguageName(translationResult.detectedLanguage)}): ${message}`);
    ...
    const expenseData = await parseExpense(translationResult.translatedText, { preferAI: false });
    ...
    if (!expenseData.amount || expenseData.amount <= 0) {
      ...
      const translatedError = await translateFromEnglish(
        errorMsg, 
    ...
    logger.info("Expense created via multilingual NLP", {
    ...
    // ============================================
    const expenses = await Expense.find({ family: req.user.family });
    const familyTotal = expenses.reduce((sum, exp) => sum + exp.amount, 0);

    // ============================================
    // STEP 3: GENERATE RESPONSE & TRANSLATE BACK
    // ============================================
    const englishResponse = generateSimpleResponse(
      expenseData.amount, 
    ...
    console.log(`🌐 Translating response back to ${getLanguageName(translationResult.detectedLanguage)}...`);
    const localizedResponse = await translateFromEnglish(
      englishResponse,
      translationResult.detectedLanguage
    );

    console.log(`✅ Final response (${getLanguageName(translationResult.detectedLanguage)}): ${localizedResponse}`);
    ...
    return res.status(201).json({
      ...
      language: {
        ...
        name: getLanguageName(translationResult.detectedLanguage)
      }
    });
    ...
    logger.error("Chat expense logging failed", {
      error: error.message,
      userId: req.user?.id
    });
 ...
 */
export const chatAboutExpenses = async (
  req: AuthRequest,
  res: Response
): Promise<void | Response> => {
  try {
    ...
    // Translate response back to user's language
    console.log(`🌐 Translating response to ${getLanguageName(translationResult.detectedLanguage)}...`);
    const localizedResponse = await translateFromEnglish(
      englishResponse,
      translationResult.detectedLanguage
    );
