 ...
 */
export const categorizeExpense = async (text: string): Promise<string> => {
 ...
 */
export const parseExpenseWithNLP = async (text: string): Promise<ParsedExpense> => {
 ...
 */
export const isExpenseEntry = (text: string): boolean => {
  const lower = text.toLowerCase();
 ...
 */
export const generateDescription = (category: string, amount: number, originalText: string): string => {
 ...
 */
export const validateExpense = (expense: ParsedExpense): { valid: boolean; errors: string[] } => {
  const errors: string[] = [];
